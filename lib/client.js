...
/* Chat mode: keep the section header but show only the search — always
   expanded (no icon), hiding the "Workspaces" label. Typing in it drives the
   stock session search (query state, not the expansion state). */
html[data-dsh-chat-mode="chat"] [class*="sectionHeader"] { display: flex !important; }
html[data-dsh-chat-mode="chat"] [class*="sectionLabel"] { display: none !important; }
html[data-dsh-chat-mode="chat"] [class*="searchSlot"] { max-width: 100% !important; }
html[data-dsh-chat-mode="chat"] [class*="searchSlot"] > [class*="search"] {
...
html[data-dsh-chat-mode="chat"] [class*="searchButton"] { display: none !important; }
html[data-dsh-chat-mode="chat"] [class*="searchInput"] {
  opacity: 1 !important;
...
/* Chat mode only: drop the collapse chevron so chat workspaces read as
...
`;
    ...
    // The sidebar search results are a flat list of sessions from every
    // workspace. Filter them per tab: Chat mode keeps only chats, Code mode
    // keeps only non-chats (the searchResultWorkspace label is "Chat · <id>").
    function filterSearchResults() {
      const chatMode = document.documentElement.getAttribute('data-dsh-chat-mode') === 'chat';
      for (const ws of document.querySelectorAll('[class*="searchResultWorkspace"]')) {
        const isChat = ws.textContent.trim().startsWith('Chat · ');
        const row = ws.closest('button[class*="searchResultRow"]');
        if (!row) continue;
        row.style.display = chatMode ? (isChat ? '' : 'none') : (isChat ? 'none' : '');
      }
    }

    // Clear the sidebar search input (React-controlled) so each tab starts
    // with its own empty search instead of sharing one query across tabs.
    ...
    function startShell(ctx, t) {
      ...
      function App({ t, anchors, revision }) {
        ...
        const [busy, setBusy] = useState(false);
        // The chat/code session the user was last viewing on each tab, so
        // flipping away and back restores it instead of opening fresh.
        const lastChatSessionId = useRef(undefined);
        const lastCodeSessionId = useRef(undefined);
        ...
        // Switching tabs also switches the main view to a session of the
        // matching kind: Chat → a chat session, Code → a code session. Each
        // direction remembers the session being left and restores it on the
        // way back (falling back to a fresh session if it's gone).
        const setMode = useCallback((next) => {
          setModeState(next);
          const snap = ctx.sessions.list.getSnapshot();
          const current = snap.current;
          if (next === 'chat') {
            if (current !== undefined && !isChatSession(ctx)) {
              lastCodeSessionId.current = current; // remember the code session left
            }
            if (isChatSession(ctx)) return; // already on a chat
            const remembered = lastChatSessionId.current;
            if (remembered !== undefined && snap.byId && snap.byId[remembered] !== undefined) {
              ctx.sessions.open(remembered);
            } else {
              newChat();
            }
          } else {
            if (current !== undefined && isChatSession(ctx)) {
              lastChatSessionId.current = current; // remember the chat left
            }
            if (current !== undefined && !isChatSession(ctx)) return; // already on code
            const remembered = lastCodeSessionId.current;
            if (remembered !== undefined && snap.byId && snap.byId[remembered] !== undefined) {
              ctx.sessions.open(remembered);
            } else {
        ...
        useEffect(() => {
          ...
          filterSearchResults(); // re-filter any open search results per tab
          clearSearch(); // make each tab's search independent
