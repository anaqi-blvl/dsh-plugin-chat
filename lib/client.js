      ...
      return h(Modal, {
        ...
        listing === null
          ...
          : h('div', { style: { display: 'flex', flexDirection: 'column', gap: 8, minWidth: 360 } },
              h('div', { style: { display: 'flex', flexWrap: 'wrap', gap: 2, color: 'var(--dsw-alias-label-secondary)', fontSize: 13 } },
                (listing.crumbs || []).map((c, i) => h('span', {
                  key: c.path,
                  onClick: () => { setError(null); setCwd(c.path); },
                  style: { cursor: 'pointer', color: 'var(--dsw-alias-state-business-primary)' },
                }, (i > 0 ? ' / ' : '') + (c.name || '/'))),
              ),
