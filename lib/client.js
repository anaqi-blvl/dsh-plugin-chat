...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const { IconNewChatOutline16, IconFolderClose16, Menu } = primitives;
...
`;
    ...
    // lower slot priority so it shadows the default WorkspacePicker.
    function FilteredWorkspacePicker({ open, anchorRef, selectedId, onPick, onClose, useWorkspaces }) {
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => ctx.slots.inject("conversation.hero.workspace", () =>
        ctx.slots.register({ name: "conversation.hero.workspace", priority: -1 }, FilteredWorkspacePicker)
