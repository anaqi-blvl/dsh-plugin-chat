...
`;
    ...
    function startShell(ctx, t) {
      ...
      function App({ t, anchors, revision }) {
        ...
        // matching kind: Chat → a chat session ("New chat"), Code → a code
        // session ("New session"). No-op when already on that kind.
        const setMode = useCallback((next) => {
          setModeState(next);
          if (next === 'chat') {
            if (!isChatSession(ctx)) newChat();
          } else {
            if (ctx.sessions.list.getSnapshot().current !== undefined && !isChatSession(ctx)) return;
