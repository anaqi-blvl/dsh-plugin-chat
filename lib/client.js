...
`;
    ...
    function startShell(ctx, t) {
      ...
      function App({ t, anchors, revision }) {
        ...
        const [busy, setBusy] = useState(false);
        // The chat session the user was last viewing on the Chat tab, so
        // flipping away to Code and back restores it instead of opening fresh.
        ...
        // matching kind: Chat → a chat session, Code → a code session. When
        // leaving Chat we remember the chat being viewed, and returning to
        // Chat restores it (falling back to a fresh chat if it's gone).
        const setMode = useCallback((next) => {
          ...
          if (next === 'chat') {
            ...
            if (current !== undefined && isChatSession(ctx)) {
              lastChatSessionId.current = current; // remember the chat being left
            }
