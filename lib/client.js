...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const { IconNewChatOutline16 } = primitives;
    ...
    const en = { "chat": "Chat", "code": "Code", "newChat": "New chat" };
    const zh = { "chat": "聊天", "code": "代码", "newChat": "新对话" };
...
/* Chat mode only: hide the hero's workspace chip + agent preset (Code
   concepts), so a fresh chat reads as a plain composer. This is mode-keyed
   (synchronous) so the chip never flashes — and never opens its picker menu
   against a display:none anchor (which would portal it to the top of the
   screen). */
html[data-dsh-chat-mode="chat"] [class*="heroWorkspaceRow"] { display: none !important; }
...
html[data-dsh-chat-session="true"] [aria-label="Choose workspace"] { display: none !important; }
...
`;
    ...
    const inject = ["locale", "workspaces", "sessions"];
    ...
    function apply(ctx) {
      ...
      // Code mode's "New Session" must not inherit a chat workspace from the
      // current chat session. Intercept the native New Session entry points
      // (logo brand + New Session button, both class-hashed) in capture phase.
      const onNewSessionClick = (e) => {
        ...
        if (btn === null) return;
        if (!isChatSession(ctx)) return; // native behavior is fine otherwise
        e.preventDefault();
        e.stopPropagation();
