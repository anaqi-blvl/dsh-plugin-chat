...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const { IconNewChatOutline16, IconFolderClose16, IconPlusOutline16, Menu } = primitives;
    ...
    const en = { "chat": "Chat", "code": "Code", "newChat": "New chat", "addWorkspace": "Add workspace" };
    const zh = { "chat": "聊天", "code": "代码", "newChat": "新对话", "addWorkspace": "添加工作区" };
...
`;
    ...
    // lower slot priority so it shadows the default WorkspacePicker. It keeps an
    // "Add workspace" footer (native pickDirectory path) so the picker doesn't
    // lose the create-a-workspace affordance.
    const ADD_WORKSPACE = "::add-workspace";

    function FilteredWorkspacePicker({ open, anchorRef, selectedId, onPick, onClose, useWorkspaces, createWorkspace, pickDirectory, t }) {
      ...
      const addEntries = [{ id: ADD_WORKSPACE, label: t('addWorkspace'), icon: h(IconPlusOutline16, { size: 16 }), disabled: busy }];
      ...
      const addWorkspace = async () => {
        ...
        try {
          ...
          console.log('[dsh-chat] add workspace failed', e && e.message);
        } finally {
          setBusy(false);
        }
      };
      return h(Menu, {
        ...
        footer: addEntries,
        selectedId,
        onSelect: (id) => { if (id === ADD_WORKSPACE) { addWorkspace(); return; } onPick(id); },
        onClose,
        side: 'bottom',
        portal: true,
        getAnchorRect,
    ...
    // Hide the session stats line ("N turns · M steps …") for chat sessions —
    // its class is build-hashed, so match by content.
    function hideChatStatusLine() {
      for (const el of document.querySelectorAll('[class*="root"]')) {
        const t = (el.textContent || '').trim();
        if (t.length > 0 && t.length < 120 && /^\d+\s+turns/i.test(t)) {
    ...
    function startShell(ctx, t) {
      ...
      function App({ t, anchors, revision }) {
        ...
        const [mode, setModeState] = useState(() => {
          try { return localStorage.getItem('dsh-chat.mode') === 'code' ? 'code' : 'chat'; } catch { return 'chat'; }
        });
        ...
        const setMode = useCallback((next) => {
          setModeState(next);
          try { localStorage.setItem('dsh-chat.mode', next); } catch { /* ignore */ }
        }, []);
    ...
    function apply(ctx) {
      ...
      // Apply the persisted mode synchronously (before React mounts), so the
      // tab-dependent sidebar filtering has no first-paint flash.
      let initialMode = 'chat';
      try { initialMode = localStorage.getItem('dsh-chat.mode') === 'code' ? 'code' : 'chat'; } catch { /* ignore */ }
      document.documentElement.setAttribute('data-dsh-chat-mode', initialMode);
      ...
      const domObserver = new MutationObserver(() => {
        ...
        // so hiding the chat entries here happens before it is ever visible
        // (no "long list" flash). The heavier attribute/status detect stays
        // debounced.
        hideChatPickerEntries();
        scheduleDetect();
      });
