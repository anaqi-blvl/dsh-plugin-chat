...
`;
    ...
    // In Chat mode, the sidebar search results are a flat list of sessions from
    // every workspace. Drop the non-chat rows so searching the chat tab only
    // surfaces chats (the searchResultWorkspace label is "Chat · <id>").
    function hideNonChatSearchResults() {
      for (const ws of document.querySelectorAll('[class*="searchResultWorkspace"]')) {
        if (ws.textContent.trim().startsWith('Chat · ')) continue;
